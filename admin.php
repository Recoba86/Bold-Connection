...
if (in_array($text, $textadmin) || $datain == "admin") {
    ...
    $Balanceall = select("user", "SUM(Balance) AS total_balance", null, null, "select")['total_balance'];
    ...
    $agentsumn = select("user", "COUNT(id) AS total_agents", "agent", "n", "select")['total_agents'];
    $agentsumn2 = select("user", "COUNT(id) AS total_agents", "agent", "n2", "select")['total_agents'];
    ...
    $sql = "SELECT SUM(price_product) AS total_price FROM invoice WHERE (time_sell BETWEEN :requestedDate AND :requestedDateend) AND (status = 'active' OR status = 'end_of_time'  OR status = 'end_of_volume' OR Status = 'send_on_hold' OR Status = 'sendedwarn') AND name_product != 'سرویس تست'";
...
{$userdata['text']}";
    ...
    $stmt = $pdo->prepare("INSERT IGNORE INTO help (name_os) VALUES (:name_os)");
    $stmt->execute([':name_os' => $text]);
...
👤 ایدی عددی کاربر: {$Payment_report['id_user']}";
    ...
    $getdataproduct = $pdo->prepare("SELECT * FROM product WHERE (Location = :location OR Location = '/all') AND agent = :agent");
    ...
    $panel = select("marzban_panel", "*", "code_panel", $user['Processing_value_one'], "select");
    $stmt = $pdo->prepare("SELECT * FROM product WHERE id = :id_product AND agent = :agent AND (Location = :location OR Location = '/all') LIMIT 1");
    $stmt->execute([
...
خیر : 0", $backadmin, 'HTML');
    ...
    $stmt->execute(array_merge($query_params, [
...
موجودی کاربر پس از کم کردن : $Balance_user_afters";
    ...
    $date = date("Y-m-d");
    $stmt = $pdo->prepare("SELECT COUNT(*) AS total_orders FROM invoice WHERE (status = 'active' OR status = 'end_of_time'  OR status = 'end_of_volume' OR status = 'sendedwarn' OR Status = 'send_on_hold') AND id_user = :id_user");
    $stmt->execute([':id_user' => $id_user]);
    $dayListSell = $stmt->fetch(PDO::FETCH_ASSOC);
    $stmt = $pdo->prepare("SELECT SUM(price) AS total_paid FROM Payment_report WHERE payment_Status = 'paid' AND id_user = :id_user AND Payment_Method != 'low balance by admin'");
    $stmt->execute([':id_user' => $id_user]);
    $balanceall = $stmt->fetch(PDO::FETCH_ASSOC);
    $stmt = $pdo->prepare("SELECT SUM(price_product) AS total_purchase FROM invoice WHERE (status = 'active' OR status = 'end_of_time'  OR status = 'end_of_volume' OR status = 'sendedwarn' OR Status = 'send_on_hold') AND id_user = :id_user");
    $stmt->execute([':id_user' => $id_user]);
    $subbuyuser = $stmt->fetch(PDO::FETCH_ASSOC);
    $invoicecount = select("invoice", '*', "id_user", $id_user, "count");
    if ($invoicecount == 0) {
        $sumvolume['total_volume'] = 0;
    } else {
        $stmt = $pdo->prepare("SELECT SUM(Volume) AS total_volume FROM invoice WHERE (status = 'active' OR status = 'end_of_time'  OR status = 'end_of_volume' OR status = 'sendedwarn' OR Status = 'send_on_hold') AND id_user = :id_user AND name_product != 'سرویس تست'");
        $stmt->execute([':id_user' => $id_user]);
        $sumvolume = $stmt->fetch(PDO::FETCH_ASSOC);
    }
    ...
    if ($subbuyuser['total_purchase'] == null)
        $subbuyuser['total_purchase'] = 0;
    $keyboardmanage = [
        'inline_keyboard' => [
    ...
    $sql = "SELECT SUM(price_product) AS total_price FROM invoice WHERE time_sell > :requestedDate AND (Status = 'active' OR Status = 'end_of_time'  OR Status = 'end_of_volume' OR status = 'sendedwarn' OR Status = 'send_on_hold') AND name_product != 'سرویس تست' AND id_user = :id_user";
    $stmt = $pdo->prepare($sql);
    $stmt->bindParam(':id_user', $id_user);
    $stmt->bindParam(':requestedDate', $desired_date_time_start);
    $stmt->execute();
...
⭕️  مجموع حجم خریداری شده فعال ( برای آمار دقیق حجم باید کرون روشن باشد): {$sumvolume['total_volume']}
$text_expie_agent

💎 گزارشات مالی

🔰 موجودی کاربر : {$user['Balance']}
🔰 تعداد خرید کل کاربر : {$dayListSell['total_orders']}
🔰️ مبلغ کل پرداختی  :  {$balanceall['total_paid']}
🔰 جمع کل خرید : {$subbuyuser['total_purchase']}
...
⚠️ توجه داشته باشید شما می توانید چندین شماره کارت تعریف کنید در صورت تعریف چندین شماره کارت به کاربر یک شماره کارت از بین شماره کارت ها رندوم نشان خواهد داد";
    ...
    try {
        ...
        $stmt = $pdo->prepare("INSERT INTO card_number (cardnumber, namecard) VALUES (:cardnumber, :namecard)");
        $stmt->execute([
            ':cardnumber' => $user['Processing_value'],
            ':namecard' => $text,
        ]);
...
مرچنت کد فعلی شما : $PaySetting";
    ...
    if ($marzban_list_get['type'] == "marzban") {
        ...
        if (isset($Check_token['access_token'])) {
            ...
            $stmt = $pdo->prepare("SELECT COUNT(*) AS total_sales, SUM(price_product) AS total_price FROM invoice WHERE (status = 'active' OR status = 'end_of_time'  OR status = 'end_of_volume' OR status = 'sendedwarn' OR Status = 'send_on_hold') AND Service_location = :service_location AND name_product != 'سرویس تست'");
            $stmt->execute([':service_location' => $marzban_list_get['name_panel']]);
            $panelSales = $stmt->fetch(PDO::FETCH_ASSOC);
            $ListSell = number_format($panelSales['total_sales'] ?? 0);
            $ListSellSUM = number_format($panelSales['total_price'] ?? 0);
...
<code>تایید</code>", $backadmin, 'HTML');
    ...
    $result = $pdo->prepare("SELECT * FROM user LIMIT :start_index, :items_per_page");
    $result->bindValue(':start_index', (int) $start_index, PDO::PARAM_INT);
    $result->bindValue(':items_per_page', (int) $items_per_page, PDO::PARAM_INT);
    $result->execute();
    $keyboardlists = [
        'inline_keyboard' => [],
    ];
    ...
    $start_index = ($page - 1) * $items_per_page;
    if ($typeagent == "all") {
        $result = $pdo->prepare("SELECT * FROM user WHERE agent != 'f' LIMIT :start_index, :items_per_page");
        $result->bindValue(':start_index', (int) $start_index, PDO::PARAM_INT);
        $result->bindValue(':items_per_page', (int) $items_per_page, PDO::PARAM_INT);
        $result->execute();
    } else {
        $result = $pdo->prepare("SELECT * FROM user WHERE agent = :agent LIMIT :start_index, :items_per_page");
        $result->bindValue(':agent', $typeagent, PDO::PARAM_STR);
        $result->bindValue(':start_index', (int) $start_index, PDO::PARAM_INT);
        $result->bindValue(':items_per_page', (int) $items_per_page, PDO::PARAM_INT);
        $result->execute();
    }
    ...
    $items_per_page = 10;
    $start_index = ($page - 1) * $items_per_page;
    $result = $pdo->prepare("SELECT * FROM user WHERE Balance != '0' LIMIT :start_index, :items_per_page");
    $result->bindValue(':start_index', (int) $start_index, PDO::PARAM_INT);
    $result->bindValue(':items_per_page', (int) $items_per_page, PDO::PARAM_INT);
    $result->execute();
    $keyboardlists = [
    ...
    $items_per_page = 10;
    $start_index = ($page - 1) * $items_per_page;
    $result = $pdo->prepare("SELECT * FROM user WHERE affiliatescount != '0' LIMIT :start_index, :items_per_page");
    $result->bindValue(':start_index', (int) $start_index, PDO::PARAM_INT);
    $result->bindValue(':items_per_page', (int) $items_per_page, PDO::PARAM_INT);
    $result->execute();
    $keyboardlists = [
    ...
    $randomString = bin2hex(random_bytes(5));
    $stmt = $pdo->prepare("INSERT INTO Payment_report (id_user, id_order, time, price, payment_Status, Payment_Method, id_invoice) VALUES (:id_user, :id_order, :time, :price, :payment_status, :payment_method, :id_invoice)");
    $payment_Status = "paid";
...
👤 نام کاربری کانفیگ: {$extend['username']}";
    ...
    $stmt = $pdo->prepare("SELECT SUM(price) AS total_price FROM service_other WHERE username = :username AND type != 'change_location' AND type != 'extend_user' LIMIT 1");
    ...
    if ($DataUserOut['status'] == "on_hold") {
        ...
        if ($serviceTime > 0) {
            $pricetime = ($nameloc['price_product'] / $serviceTime) + intval($sumproduct['total_price']);
        ...
        if ($dataLimit > 0) {
            ...
            $pricelast = round($volumeleft * ($nameloc['price_product'] + intval($sumproduct['total_price'])), 2);
        } else {
            $pricelast = 0;
        }
    } else {
        ...
        if ($serviceTime > 0 && $volumeDivisor > 0) {
            ...
            $pricelast = round($timeleft * $volumeleft * ($nameloc['price_product'] + intval($sumproduct['total_price'])), 2);
        } else {
            $pricelast = 0;
        }
    }
...
trojan://xyz", $backadmin, 'HTML');
    ...
    $stmt = $pdo->prepare("DELETE FROM manualsell WHERE namerecord = :namerecord");
    $stmt->execute([':namerecord' => $text]);
    step("home", $from_id);
...
f,n.n2", $backadmin, 'HTML');
    ...
    $userdata = json_decode($user['Processing_value'], true);
    $stmt = $pdo->prepare("SELECT * FROM product WHERE Location = :location AND agent = :agent");
    $stmt->execute([
        ':location' => $userdata['namepanel'],
        ':agent' => $userdata['agent'],
    ]);
...
f,n.n2", $backadmin, 'HTML');
    ...
    $stmt = $pdo->prepare("DELETE FROM departman WHERE name_departman = :name_departman");
    $stmt->execute([':name_departman' => $text]);
...
"
    ...
    $items_per_page = 10;
    $start_index = ($page - 1) * $items_per_page;
    $result = $pdo->prepare("SELECT * FROM invoice WHERE id_user = :id_user LIMIT :start_index, :items_per_page");
    ...
    $items_per_page = 10;
    $start_index = ($page - 1) * $items_per_page;
    $result = $pdo->prepare("SELECT * FROM user WHERE cardpayment = '1' LIMIT :start_index, :items_per_page");
    $result->bindValue(':start_index', (int) $start_index, PDO::PARAM_INT);
    $result->bindValue(':items_per_page', (int) $items_per_page, PDO::PARAM_INT);
    $result->execute();
    $keyboardlists = [
