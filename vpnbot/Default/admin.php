...
👤 ایدی عددی کاربر: {$Payment_report['id_user']}";
    ...
    $date = date("Y-m-d");
    $stmt = $pdo->prepare("SELECT COUNT(*) AS total FROM invoice WHERE (status = 'active' OR status = 'end_of_time' OR status = 'end_of_volume' OR status = 'sendedwarn' OR Status = 'send_on_hold') AND id_user = :id_user AND bottype = :ApiToken");
    $stmt->bindParam(':id_user', $id_user);
    $stmt->bindParam(':ApiToken', $ApiToken);
    $stmt->execute();
    $dayListSell = $stmt->fetch(PDO::FETCH_ASSOC);

    $stmt = $pdo->prepare("SELECT SUM(price) AS total FROM Payment_report WHERE payment_Status = 'paid' AND id_user = :id_user AND Payment_Method != 'low balance by admin' AND bottype = :ApiToken");
    $stmt->bindParam(':id_user', $id_user);
    $stmt->bindParam(':ApiToken', $ApiToken);
    $stmt->execute();
    $balanceall = $stmt->fetch(PDO::FETCH_ASSOC);

    $stmt = $pdo->prepare("SELECT SUM(price_product) AS total FROM invoice WHERE (status = 'active' OR status = 'end_of_time' OR status = 'end_of_volume' OR status = 'sendedwarn' OR Status = 'send_on_hold') AND id_user = :id_user AND bottype = :ApiToken");
    $stmt->bindParam(':id_user', $id_user);
    $stmt->bindParam(':ApiToken', $ApiToken);
    $stmt->execute();
    ...
    if ($invoicecount == 0) {
        ...
        $stmt = $pdo->prepare("SELECT SUM(Volume) AS total FROM invoice WHERE (status = 'active' OR status = 'end_of_time' OR status = 'end_of_volume' OR status = 'sendedwarn' OR Status = 'send_on_hold') AND id_user = :id_user AND name_product != 'سرویس تست'");
        $stmt->bindParam(':id_user', $id_user);
        $stmt->execute();
...
⭕️  مجموع حجم خریداری شده فعال ( برای آمار دقیق حجم باید کرون روشن باشد): {$sumvolume['total']}

💎 گزارشات مالی

🔰 موجودی کاربر : $userbalance
🔰 تعداد خرید کل کاربر : {$dayListSell['total']}
🔰️ مبلغ کل پرداختی  :  {$balanceall['total']}
🔰 جمع کل خرید : {$subbuyuser['total']}
...
";
    ...
    $product = [];
    $stmt = $pdo->prepare("SELECT * FROM product WHERE agent = :agent");
