...
if ($user['step'] == "createusertest" || preg_match('/locationtest_(.*)/', $datain, $dataget) || ($text == $text_bot_var['btn_keyboard']['test'])) {
    ...
    $stmt = $pdo->prepare("INSERT IGNORE INTO invoice (id_user, id_invoice, username, time_sell, Service_location, name_product, price_product, Volume, Service_time, Status, bottype, notifctions) VALUES (:id_user, :id_invoice, :username, :time_sell, :service_location, :name_product, :price_product, :volume, :service_time, :status, :bottype, :notifctions)");
    $Status = "active";
...
if ($text == $text_bot_var['btn_keyboard']['buy'] && $setting['active_step_note']) {
    ...
    $locationproduct = $pdo->prepare("SELECT * FROM marzban_panel WHERE status = 'active' AND (agent = :agent OR agent = 'all')");
    $locationproduct->execute([':agent' => $userbot['agent']]);
    $locationRows = $locationproduct->fetchAll(PDO::FETCH_ASSOC);
    if (count($locationRows) == 0) {
        sendmessage($from_id, $textbotlang['Admin']['managepanel']['nullpanel'], null, 'HTML');
        return;
    }
    if (count($locationRows) == 1) {
        $location = $locationRows[0]['name_panel'];
...
💰 سفارش شما آماده پرداخت است";
    ...
    $stmt = $pdo->prepare("INSERT IGNORE INTO invoice (id_user, id_invoice, username, time_sell, Service_location, name_product, price_product, Volume, Service_time, Status, bottype, note, notifctions) VALUES (:id_user, :id_invoice, :username, :time_sell, :service_location, :name_product, :price_product, :volume, :service_time, :status, :bottype, :note, :notifctions)");
    $Status = "unpaid";
    $stmt->execute([
        ':id_user' => $from_id,
        ':id_invoice' => $randomString,
...
▫️زمان خرید : $timejalali";
    ...
    $stmt = $pdo->prepare("INSERT INTO Payment_report (id_user, id_order, time, price, payment_Status, Payment_Method, id_invoice, bottype) VALUES (:id_user, :id_order, :time, :price, :payment_status, :payment_method, :id_invoice, :bottype)");
...
دلیل خطا : {$extend['msg']}";
    ...
    $stmt->execute([
        ...
        ':type' => $type,
        ':time' => $dateacc,
        ':price' => $datafactor['price_product'],
