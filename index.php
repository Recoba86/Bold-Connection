...
با خریدهای این کاربر، <b>سهم هدیه شما</b> به حسابت واریز می‌شه 🔥", $keyboard, 'html');
            ...
            $stmt = $pdo->prepare("INSERT IGNORE INTO reagent_report (user_id, get_gift, time, reagent) VALUES (:user_id, :get_gift, :time, :reagent)");
...
if ($user['step'] == "createusertest" || preg_match('/locationtest_(.*)/', $datain, $dataget) || ($text == $datatextbot['text_usertest'] || $datain == "usertestbtn" || $text == "usertest")) {
    ...
    $stmt->execute([
        ...
        ':volume' => $marzban_list_get['val_usertest'],
        ':service_time' => $marzban_list_get['time_usertest'],
        ':status' => $Status,
...
📆 $dateacc → ⏰ $timeacc
    ...
    $stmt->execute();
    $locationproduct = $stmt->fetchAll(PDO::FETCH_ASSOC);
    if (count($locationproduct) == 0) {
        sendmessage($from_id, $textbotlang['Admin']['managepanel']['nullpanel'], null, 'HTML');
        return;
    }
    if ($setting['get_number'] == "onAuthenticationphone" && $user['step'] != "get_number" && $user['number'] == "none") {
    ...
    if ($user['number'] == "none" && $setting['get_number'] == "onAuthenticationphone")
        return;
    #-----------------------#
    if (count($locationproduct) == 1) {
        $location = $locationproduct[0]['name_panel'];
...
⚠️ حداقل زمان $maintime روز  و حداکثر $maxtime روز  می توانید تهیه کنید";
    ...
    if ($parts[0] == "customvolume") {
        ...
        $stmt = $pdo->prepare("SELECT * FROM product WHERE code_product = :code_product AND (Location = :location OR Location = '/all') LIMIT 1");
        $stmt->bindValue(':code_product', $loc, PDO::PARAM_STR);
    ...
    $stmt = $pdo->prepare("INSERT IGNORE INTO invoice (id_user, id_invoice, username, time_sell, Service_location, name_product, price_product, Volume, Service_time, Status, note, refral, notifctions) VALUES (:id_user, :id_invoice, :username, :time_sell, :service_location, :name_product, :price_product, :volume, :service_time, :status, :note, :refral, :notifctions)");
    $Status = "unpaid";
    $stmt->execute([
        ':id_user' => $from_id,
        ':id_invoice' => $randomString,
...
نام پنل : {$marzban_list_get['name_panel']}";
        ...
        $stmt->execute([
            ...
            ':time_sell' => $date,
            ':service_location' => $user['Processing_value'],
            ':name_product' => $info_product['name_product'],
            ':price_product' => $info_product['price_product'],
            ':volume' => $info_product['Volume_constraint'],
            ':service_time' => $info_product['Service_time'],
...
💬 مبلغ باید عددی صحیح بین $minbalanceFmt تا $maxbalanceFmt تومان باشد", null, 'HTML');
    ...
    if ($datain == "cart_to_offline") {
        ...
        $stmt = $pdo->prepare("SELECT * FROM Payment_report WHERE id = :id AND payment_Status = 'Unpaid'");
        $stmt->bindValue(':id', $from_id, PDO::PARAM_STR);
        $stmt->execute();
        if ($stmt->rowCount() != 0) {
        ...
        try {
            $stmt = $pdo->prepare("SELECT * FROM card_number ORDER BY RAND() LIMIT 1");
            $stmt->execute();
            $card_info = $stmt->fetch(PDO::FETCH_ASSOC);
        } catch (\PDOException $e) {
            error_log('Failed to fetch card_number data: ' . $e->getMessage());
...
نام کاربری کاربر : @$username";
        ...
        $stmt->execute([
            ...
            ':payment_method' => $Payment_Method,
            ':id_invoice' => $invoice,
            ':dec_not_confirmed' => $pay['data']['authority'],
        ]);
        $paymentkeyboard = json_encode([
            'inline_keyboard' => [
                [
...
if (preg_match('/Confirmpay_user_(\w+)_(\w+)/', $datain, $dataget)) {
    ...
    $id_order = $dataget[2];
    $stmt = $pdo->prepare("SELECT * FROM Payment_report WHERE id_order = :id_order LIMIT 1");
...
✍️ در صورت درست بودن رسید پرداخت را تایید نمایید.";
        ...
        if ($codeproduct == "custom_volume") {
            ...
            $stmt->bindValue(':agent', $user['agent'], PDO::PARAM_STR);
            $stmt->bindValue(':code_product', $codeproduct, PDO::PARAM_STR);
            $stmt->execute();
            $prodcut = $stmt->fetch(PDO::FETCH_ASSOC);
        }
        $Confirm_pay = json_encode([
...
دلیل خطا : {$extra_volume['msg']}";
    ...
    $stmt->bindValue(':id_user', $from_id, PDO::PARAM_STR);
    $stmt->bindValue(':start_index', (int)$start_index, PDO::PARAM_INT);
    ...
    $start_index = ($previous_page - 1) * $items_per_page;
    $stmt = $pdo->prepare("SELECT * FROM invoice WHERE id_user = :id_user AND (status = 'active' OR status = 'end_of_time' OR status = 'end_of_volume' OR status = 'sendedwarn' OR Status = 'send_on_hold') ORDER BY time_sell DESC LIMIT :previous_page, :items_per_page");
    $stmt->bindValue(':id_user', $from_id, PDO::PARAM_STR);
    $stmt->bindValue(':previous_page', (int)$previous_page, PDO::PARAM_INT);
    $stmt->bindValue(':items_per_page', (int)$items_per_page, PDO::PARAM_INT);
    $stmt->execute();
    $keyboardlists = [
        'inline_keyboard' => [],
