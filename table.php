...
try {
    ...
    if (!$tableExists) {
        ...
        $stmt = $pdo->prepare("INSERT INTO admin (id_admin, rule, username, password) VALUES (:id_admin, :rule, :username, :password)");
        $stmt->execute([
            ':id_admin' => $adminnumber,
            ':rule' => 'administrator',
            ':username' => 'admin',
...
try {
    ...
    if (!$table_exists) {
        ...
        $updateCodePanel = $pdo->prepare("UPDATE marzban_panel SET code_panel = :code_panel WHERE id = :id");
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            $code = '7e' . $next_num;
            $updateCodePanel->execute([
                ':code_panel' => $code,
                ':id' => $row['id'],
            ]);
            $next_num++;
...
try {
    ...
    if (!$table_exists) {
        ...
        if ($Check_filde->rowCount() != 1) {
            ...
            $stmt = $pdo->prepare("UPDATE invoice SET notifctions = :notifctions");
            $stmt->execute([':notifctions' => $data]);
        }
...
//-----------------------------------------------------------------
try {
    $result = $pdo->query("SHOW TABLES LIKE 'textbot'");
    $table_exists = ($result->rowCount() > 0);
    $text_roll = "
...
🔝لزومی به ارسال رسید نیست، اما در صورتی که بعد از گذشت مدتی واریز شما تایید نشد، عکس رسید خود را ارسال کنید.";
    ...
    if (!$table_exists) {
        $result = $pdo->query("CREATE TABLE textbot (
        id_text varchar(600) PRIMARY KEY NOT NULL,
        text TEXT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL)
        ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE utf8mb4_unicode_ci");

        foreach ($insertQueries as $query) {
            $stmt = $pdo->prepare("INSERT INTO textbot (id_text, text) VALUES (:id_text, :text)");
            $stmt->execute([
                ':id_text' => $query[0],
                ':text' => $query[1],
            ]);
        ...
        foreach ($insertQueries as $query) {
            $stmt = $pdo->prepare("INSERT IGNORE INTO textbot (id_text, text) VALUES (:id_text, :text)");
            $stmt->execute([
                ':id_text' => $query[0],
                ':text' => $query[1],
            ]);
...
try {
    ...
    if (!$table_exists) {
        ...
        foreach ($settings as $setting) {
            $stmt = $pdo->prepare("INSERT INTO PaySetting (NamePay, ValuePay) VALUES (:name_pay, :value_pay)");
            $stmt->execute([
                ':name_pay' => $setting[0],
                ':value_pay' => $setting[1],
            ]);
        ...
        foreach ($settings as $setting) {
            $stmt = $pdo->prepare("INSERT IGNORE INTO PaySetting (NamePay, ValuePay) VALUES (:name_pay, :value_pay)");
            $stmt->execute([
                ':name_pay' => $setting[0],
                ':value_pay' => $setting[1],
            ]);
...
try {
    ...
    if (!$tableExists) {
        ...
        $stmt->execute();
        $stmt = $pdo->prepare("INSERT INTO departman (idsupport, name_departman) VALUES (:idsupport, :name_departman)");
        $stmt->execute([
            ':idsupport' => $adminnumber,
            ':name_departman' => '☎️ بخش عمومی',
        ]);
    }
} catch (PDOException $e) {
    error_log('table.php migration error: ' . $e->getMessage());
