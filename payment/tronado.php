...
if ($Payment_report['payment_Status'] != "paid") {
    ...
    $response = is_string($response['body'] ?? null) ? json_decode($response['body'], true) : false;

    if ($response && !empty($response['IsPaid']) && !empty($data['IsPaid']) && ($data['TronAmount'] ?? null) == ($response['TronAmount'] ?? null)) {
        echo json_encode(array("status" => true));
        $textbotlang = languagechange('../text.json');

        $confirmation = confirmPaymentAtomically($data['PaymentID'], [
            'provider' => 'tronado',
            'payload' => $data,
            'remote' => $response,
        ], "../images.jpg");
        ...
        if (!empty($confirmation['confirmed']) && is_array($confirmation['payment_report'] ?? null)) {
            ...
            $balancelow = "";
            if (($data['TronAmount'] ?? 0) < ($data['ActualTronAmount'] ?? 0)) {
